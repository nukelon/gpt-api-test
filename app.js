...
function renderRawOutputBlock(rawOutput) {
...
function renderAssistantMessage(message) {
  if (message.status === "loading") {
    return `
      <article class="message assistant" data-message-id="${message.id}">
        <div class="avatar">AI</div>
        <div class="message-content">
          <div class="message-main-text">正在调用 Image API…</div>
          ${renderMessageFooter(message)}
        </div>
      </article>
    `;
  ...
  const rawOutput = images.length ? "" : renderRawOutputBlock(message.rawOutput);
  const meta = renderResponseMeta(message);
  return `
    <article class="message assistant" data-message-id="${message.id}">
      <div class="avatar">AI</div>
  ...
  return `
    <article class="message error" data-message-id="${message.id}">
      ...
      <div class="message-content">
        ...
        ${renderRawOutputBlock(message.rawOutput)}
        ${renderMessageFooter(message)}
      </div>
    </article>
  `;
...
function updateSendState() {
  ...
  const missingPromptOnly = result.errors.length === 1 && result.errors[0].includes("请输入 prompt");
  const displayErrors = missingPromptOnly && !els.promptInput.value.trim() ? [] : result.errors;
  if (pending) displayErrors.push("当前对话仍有请求进行中；如需并发，请新建一个对话再发送。");
  showValidation({ errors: displayErrors, warnings: result.warnings });
...
async function handleSend() {
  ...
  try {
    ...
    toast(result.images.length ? "图片已生成并保存到本地历史。" : "接口成功返回，但未检测到图片数据。", result.images.length ? "success" : "info");
  } catch (error) {
    const errorMessage = {
      id: assistantMessage.id,
      role: "error",
      ...
      responseMeta: { requestId: error.requestId || "" },
      createdAt: nowIso(),
    };
    replaceMessage(conversation, assistantMessage.id, errorMessage);
    await saveConversation(conversation);
...
function makeBuffer(size, writer) {
...
function createZipBlob(files) {
  ...
  for (const file of files) {
    ...
    const localHeader = makeBuffer(30, (view) => {
    ...
    const centralHeader = makeBuffer(46, (view) => {
...
async function inflateRaw(bytes) {
  if (typeof DecompressionStream !== "function") {
    throw new Error("此浏览器不支持解压压缩 ZIP。请导入本应用导出的 ZIP，或导入单个 JSON 文件。");
  }
  const stream = new Blob([bytes]).stream().pipeThrough(new DecompressionStream("deflate-raw"));
  return new Uint8Array(await new Response(stream).arrayBuffer());
}

async function readStoredZipJsonEntries(arrayBuffer) {
  ...
  while (offset + 30 <= bytes.length) {
    ...
    const name = TEXT_DECODER.decode(bytes.slice(nameStart, nameStart + nameLength));
    const dataEnd = dataStart + compressedSize;
    if (dataEnd > bytes.length) break;
    if (name.toLowerCase().endsWith(".json")) {
      let fileBytes = bytes.slice(dataStart, dataEnd);
      if (method === 8) fileBytes = await inflateRaw(fileBytes);
      else if (method !== 0) throw new Error(`ZIP 内文件 ${name} 使用了不支持的压缩方式。`);
      if (uncompressedSize && fileBytes.length !== uncompressedSize && method !== 0) {
        throw new Error(`ZIP 内文件 ${name} 解压尺寸不匹配。`);
      }
      entries.push({ name, text: TEXT_DECODER.decode(fileBytes) });
    }
    offset = dataEnd;
    if (flags & 0x08) {
      // Skip optional data descriptor if present. This basic parser searches for the next local header.
      const next = bytes.indexOf(0x50, offset);
